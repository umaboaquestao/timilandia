...
function importSession(event) { const file = event.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => { try { const loaded = JSON.parse(reader.result); const source = Array.isArray(loaded) ? loaded : loaded?.questions; const imported = source || parseOcrSession(loaded); const questions = imported.map(normalizeQuestion).filter(Boolean); if (!questions.length || questions.length !== imported.length) throw new Error(); quizQuestions = questions; saveQuestions(); const name = loaded.name || 'Quiz TIMI importado'; const sessions = loadSessions().filter(session => session.name !== name); sessions.push({ name, questions, updatedAt: new Date().toISOString() }); localStorage.setItem(sessionsKey, JSON.stringify(sessions)); renderAdmin(); } catch { window.alert('O ficheiro não contém uma sessão JSON válida.'); } }; reader.readAsText(file); }
