...
const difficultyTimes = { easy: 120, medium: 50, hard: 30 };
...
function newGame() {
  state = {
    ...
    dog: { x: 500, y: 820, radius: 24, pulse: Math.random() * Math.PI * 2 },
    ...
    score: 0,
    distanceTravelled: 0,
    timeRemaining: difficultyTimes[selectedDifficulty],
    endReason: 'danger',
...
function update(delta) {
  ...
  const extraEnergy = Math.max(0, state.collectedEnergy - 4);
  const extraCoins = Math.max(0, state.collectedCoins - 3);
  const targetSpeed = hasInput ? Math.max(130, Math.min(360, 245 + extraEnergy * 30 - extraCoins * 25)) : 0;
  ...
  for (const hazard of state.pollution) {
    if (player.invulnerable <= 0 && distance(player, hazard) < player.radius + hazard.radius - 4) {
      ...
      state.dog.x = 500;
      state.dog.y = 820;
      if (state.lives <= 0) endGame('danger');
      break;
    }
  }
  ...
  if (state.timeRemaining <= 0) endGame('timeout');
  ...
  checkMissions();
  if (state.running && state.energy.length === 0 && state.coins.length === 0) endGame('all-items');
  updateHud();
}
...
function updateDog(delta) {
  ...
  dog.x += Math.cos(angle) * 60 * delta;
  dog.y += Math.sin(angle) * 60 * delta;
}

function endGame(reason = 'danger') {
  if (!state.running) return;
  state.running = false;
  state.endReason = reason;
  const endings = {
    missions: ['Acabaste o jogo!', 'Missões cumpridas, cidade salva e bicicleta em modo campeão. Que viagem incrível!'],
    'all-items': ['Apanhaste tudo!', 'Nem uma moeda nem uma faísca ficou para trás. A TiMIlandia está orgulhosa de ti!'],
    danger: ['Fim de viagem', 'A poluição ou o cão travaram a tua bicicleta. Recupera o fôlego e tenta outra rota.'],
    timeout: ['O tempo voou!', 'A cidade ainda precisa de ti. Escolhe uma rota mais rápida e tenta de novo.'],
  };
  const [title, message] = endings[reason];
...
function checkMissions() {
  ...
  if (state.completedMissions.size === missions.length) endGame('missions');
}
...
function updateHud() {
  ...
  const seconds = Math.min(difficultyTimes[selectedDifficulty], Math.ceil(state.timeRemaining));
